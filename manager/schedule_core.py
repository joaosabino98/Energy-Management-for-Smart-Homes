...
def update_schedule_table_job():
	ScheduleManager().populate_timetable()
...
class ScheduleManager(metaclass = Singleton):
	...
	timetable = {} # { Datetime: energy available }
	...
	def clean(self):
		print("Populating schedule table and queues.")
	...
	def schedule_execution(self, execution):
		rated_power = execution.profile.rated_power
		now = self.get_current_schedule_slot()
		available_time = self.get_available_timeslot(now, execution)
	...
	def shift_executions(self, rated_power, priority):
		...
		now = self.get_current_schedule_slot()
		power_available = self.timetable[now]
		...
		self.update_executions_in_timetable(interrupted)
	...
	def start_execution(self, execution, start_time=None):
		...
		self.add_executions_to_timetable([execution])
	...
	def interrupt_execution(self, execution):
		...
		if (self.bgsched.get_job(f"{execution.id}_finish") is not None):
			self.bgsched.remove_job(f"{execution.id}_finish")
	...
	"""
	TIMETABLE HANDLING FUNCTIONS
	"""
	"""
	add_executions_to_timetable(self, list[]) -> None

	Add executions to schedule without requiring to repopulate schedule.
	CAN support shifted executions if schedule is first updated to
	reflect change in initial timeframe (update/remove).
	"""
	def add_executions_to_timetable(self, list):
		for execution in list:
			for time in self.timetable.keys():
				...
				if (execution.start_time is not None and execution.start_time < next \
					...
					self.timetable[time] -= execution.profile.rated_power

	"""
	update_executions_in_timetable(self, list[]) -> None
	...
	"""
	def update_executions_in_timetable(self, list):
		for execution in list:
	...
	"""
	remove_executions_from_timetable(self, list[]) -> None

	Remove executions from schedule without requiring to repopulate schedule.
	Called for executions that were moved before starting (anticipated or delayed).
	"""
	def remove_executions_from_timetable(self, list):
		for execution in list:
			if (execution.is_started is False):
				for time in self.timetable.keys():
					next = time + timezone.timedelta(minutes=self.step)
	...
	"""
	def get_available_timeslot(self, minimum_time, execution) -> Datetime
	...
	"""
	def get_available_timeslot(self, minimum_time, execution):
		...
		found = 0
		for time in self.timetable.keys():
			if time >= minimum_time and self.timetable[time] >= execution.profile.rated_power:
				if (found == 0):
					slot = time
				found += 1
	...
	"""
	def generate_timezone_aware_timetable(self):
	...
	def populate_timetable(self):
		...
		self.timetable = self.generate_timezone_aware_timetable()
		for time in self.timetable.keys():
			next = time + timezone.timedelta(minutes=self.step)
